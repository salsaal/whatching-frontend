...
import {
  ...
  getBotCanvasDraft,
  getBotCanvasPublished,
  getBotSettings,
  getBotStatus,
  listKnowledgeSources,
  publishBotCanvasDraft,
...
const ensureRequiredNodes = (nodes: BuilderNode[]) => {
  ...
  if (!byTrigger.has("DEFAULT")) required.push(...defaultCanvasNodes());
...
const getDraftStateFromResponse = (
  data?: ReturnType<typeof getBotCanvasDraft> extends Promise<infer R>
    ? R
    : never
) => data?.data?.canvas?.draftState || data?.data?.draftState;

const getPublishedStateFromResponse = (
  data?: ReturnType<typeof getBotCanvasPublished> extends Promise<infer R>
    ? R
    : never
...
const draftCacheKey = (orgId?: string) =>
  orgId ? `whatching:bot-flow-draft:${orgId}` : "";

const readCachedDraft = (orgId?: string): BotCanvasDraftState | null => {
  if (typeof window === "undefined") return null;
  const key = draftCacheKey(orgId);
  if (!key) return null;
  try {
...
const writeCachedDraft = (orgId: string | undefined, draft: BotCanvasDraftState) => {
...
function FlowsBuilder() {
  ...
  const {
    ...
    queryKey: ["bot-canvas-draft", activeOrganization?._id],
    queryFn: getBotCanvasDraft,
  ...
  const {
    ...
    queryKey: ["bot-canvas-published", activeOrganization?._id],
    queryFn: getBotCanvasPublished,
    enabled: Boolean(activeOrganization?._id),
    refetchOnMount: "always"
  });
  ...
  const knowledgeSources = knowledgeData?.data?.sources || [];
  ...
  const { mutateAsync: publishDraft, isPending: isPublishing } = useMutation({
    mutationFn: () => publishBotCanvasDraft(),
    onSuccess: () => {
      refetchStatus();
  ...
  const { mutateAsync: validateDraft, isPending: isValidating } = useMutation({
    mutationFn: () => validateBotCanvas(),
    meta: { showToast: false }
  });

  const { mutate: createKnowledge, isPending: isCreatingKnowledge } =
    useMutation({
  ...
  const { mutate: uploadKnowledge, isPending: isUploadingKnowledge } =
    useMutation({
  ...
  const { mutate: reingestKnowledge } = useMutation({
  ...
  useEffect(() => {
    ...
    const cachedDraft = readCachedDraft(activeOrganization?._id);
    const draft =
    ...
    const nextNodes = useStarter ? defaultCanvasNodes() : loadedNodes;
  ...
  }, [activeOrganization?._id, draftData, setEdges, setNodes]);
  ...
  useEffect(() => {
    ...
    writeCachedDraft(activeOrganization?._id, draft);
  }, [activeOrganization?._id, buildDraftState, edges, isPublishedMode, nodes]);
  ...
  useEffect(() => {
    ...
    const saveLatestDraft = () => {
      ...
      if (!draft) return;
      writeCachedDraft(activeOrganization._id, draft);
      void saveBotCanvasDraft(draft).catch(() => undefined);
    };
  ...
  }, [activeOrganization?._id, router.events]);
          ...
          <aside className="overflow-y-auto border-r bg-muted/30 p-3 [scrollbar-color:hsl(var(--border))_transparent] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border">
            ...
            <div className="mt-4 rounded-xl border bg-white p-3">
              ...
              <div className="mt-3 space-y-2">
                ...
                />
                <Textarea
                  placeholder="Paste policy, FAQ, or support text"
              ...
              <div className="mt-3 max-h-48 space-y-2 overflow-y-auto">
                {knowledgeSources.map((source) => (
                  <div key={source._id} className="rounded-md border p-2">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex gap-1">
                        <Button
                          ...
                          variant="ghost"
                          className="size-7"
                          onClick={() => reingestKnowledge(source._id)}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => deleteKnowledge(source._id)}
                        >
          ...
          >
            {(canvasMode === "draft" ? isDraftLoading : isPublishedLoading) ? (
              ...
              <ReactFlow
                ...
                nodeTypes={nodeTypes}
                onInit={setReactFlow}
                onNodesChange={isPublishedMode ? undefined : onNodesChange}
    ...
    <aside className="overflow-y-auto border-l bg-white p-4 [scrollbar-color:hsl(var(--border))_transparent] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border">
      <div className="flex items-start justify-between gap-3">
        ...
        <Button
          ...
          disabled={readOnly || node.data.locked}
          onClick={deleteNode}
        >
          {node.data.locked ? (
      ...
      <div className="mt-4 space-y-4">
        ...
        <Field label="Trigger key">
          <Input
            value={node.data.triggerKey}
            disabled={readOnly || node.data.locked}
            onChange={(event) =>
              updateNode(node.id, (data) => ({
                ...data,
                triggerKey: slugifyTrigger(event.target.value)
              }))
            }
          />
        </Field>
              ...
              return (
                ...
                >
                  ...
                  <Input
                    ...
                    placeholder="Webhook row id"
                    onChange={(event) => {
                      if (action) removeEdgesForAction(action.actionId);
                      const nextId = slugifyTrigger(event.target.value);
                      updateRow(sectionIndex, rowIndex, {
                        id: nextId,
                        replyId: nextId
                      });
